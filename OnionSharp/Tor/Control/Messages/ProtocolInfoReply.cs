...
using OnionSharp.Tor.Control.Utils;
using System.Collections.Immutable;

namespace OnionSharp.Tor.Control.Messages;
    ...
    private static int ParseProtocolVersionLine(string line)
    {
        ...
        if (token == LineTypeProtocolInfo)
        {
            if (int.TryParse(remainder, out int parsedVersion))
            {
                if (parsedVersion != 1)
                {
                    Console.WriteLine($"Version {parsedVersion} may not work expected.");
                }
            ...
            {
                Console.WriteLine("Failed to parse PROTOCOLINFO version.");
            }
        }
