    ...
    private async Task RestartingLoopForBundledTorAsync(CancellationToken globalCancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (isAlreadyRunning)
                {
                    Console.WriteLine($"Tor is already running on {_settings.SocksEndpoint}");
                ...
                Console.WriteLine("Tor is running.");
            ...
            {
                Console.WriteLine("Tor control failed to initialize.", ex);
